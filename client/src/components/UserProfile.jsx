          ...
          isEditing ? (
            ...
            <>
              <div className="relative">
                <img src="background.jpg" alt="Sunset background" className="w-full h-48 object-center rounded-lg" />
                <div className="absolute bottom-0 left-4 transform translate-y-1/2">
                  <img src="/Avatars/Avatar1.jpg" alt="Profile" className="w-32 h-32 rounded-full border-4 border-gray-900" />
                </div>
              </div>
              <div className="mt-12 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-lg text-mine-shaft-200"> <span className='font-semibold text-xl text-cyan-/-aqua-500'>{profileData.company || 'N/A'}</span> • {profileData.location}</p>
                  
                  <p className="text-lg text-cyan-/-aqua-500 cursor-pointer hover:text-cyan-/-aqua-600">{profileData.email || 'N/A'}</p>
                </div>
                <button
              ...
              <div className="mt-2">
               ...
               {profileData.skills.map((skill,index)=>(
                <span key={index} className='bg-none border border-cyan-/-aqua-600 hover:bg-cyan-/-aqua-500 hover:translate-x-1 transition duration-300 ease-in-out text-cyan-/-aqua-100 px-3 py-1 rounded-full text-md hover:cursor-pointer'>{skill}</span>
               ))}
                ...
                {profileData.experiences.length > 0 ? (
                  profileData.experiences.map((exp, index) => (
                    ...
                    <div key={index} className="mb-6 flex items-start space-x-4">
                        ...
                        <img src={`/companies/${exp.company}.png`} alt={exp.company.charAt(0)} className="w-full h-full" />
                        </div>
                        <div className='flex-1'>
                        <h3 className="text-2xl font-semibold text-cyan-/-aqua-500">{exp.title || 'N/A'}</h3>
                      <p className="text-xl text-gray-300">
                        <span className="font-medium">{exp.company || 'N/A'}</span> &middot; {exp.location || 'N/A'}
                      </p>
                      <p className="text-md text-mine-shaft-200">{exp.startDate} - {exp.endDate || 'Present'}</p>
          <p className="mt-2 text-lg text-mine-shaft-100">{exp.description || 'No description provided'}</p>
                        </div>
                    </div>
                  ))
                ) : (
